...
// Mock realistic faculty data for demo purposes
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('[API] Fetching faculty dashboard data');

    try {
      // Try to forward to backend faculty dashboard endpoint
      const response = await fetch(`${BACKEND_URL}/api/faculty-services/dashboard`, {
        method: 'GET',
        headers: {
      ...
      // If backend is not reachable, return mock data
      console.log('[API] Backend connection failed, using mock data:', backendError);
      return NextResponse.json(mockFacultyData);
    }
  } catch (error) {
