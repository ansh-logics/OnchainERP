...
// Helper function to get model (MongoDB or Sequelize)
const getModel = (modelName) => {
  if (useMongoForModel(modelName)) {
    return mongoModels[modelName];
  }
  // Fallback to Sequelize models from the shared models index
  const sequelizeModels = require('../../shared/db/models');
  return sequelizeModels[modelName];
};

// Helper function to convert Sequelize Op to MongoDB queries
const convertOp = (where) => {
  if (!where || typeof where !== 'object') return where;
  
  const converted = {};
  for (const [key, value] of Object.entries(where)) {
    if (key === Op.and || key === '$and') {
      converted.$and = Array.isArray(value) ? value.map(convertOp) : [convertOp(value)];
    } else if (key === Op.or || key === '$or') {
      converted.$or = Array.isArray(value) ? value.map(convertOp) : [convertOp(value)];
    } else if (key === Op.in || key === '$in') {
      converted.$in = value;
    } else if (key === Op.ne || key === '$ne') {
      converted.$ne = value;
    } else if (key === Op.like || key === '$like') {
      converted.$regex = new RegExp(value.replace(/%/g, '.*'), 'i');
    } else if (typeof value === 'object' && value !== null) {
      converted[key] = convertOp(value);
    } else {
      converted[key] = value;
    }
  }
...
const getAllFacultyAssignments = asyncHandler(async (req, res) => {
  try {
    ...
    if (req.user && req.user.id) {
      ...
      let firstFaculty;
      if (useMongoForModel('Faculty')) {
        firstFaculty = await FacultyModel.findOne({}).populate({
          path: 'userId',
          model: 'User',
          select: 'id'
        });
      } else {
        firstFaculty = await FacultyModel.findOne({ 
          include: [{
            model: UserModel,
            as: 'user',
            attributes: ['id']
    ...
    const { page = 1, limit = 10, status, sectionId, search } = req.query;
    const offset = (parseInt(page) - 1) * parseInt(limit);

    // Build where clause
    const whereClause = { facultyId: faculty.id };
    ...
    const SectionModel = getModel('Section');
    const AssignmentSubmissionModel = getModel('AssignmentSubmission');
    
    let assignments, totalAssignments;

    if (useMongoForModel('Assignment')) {
      // MongoDB queries
      const mongoWhere = convertOp(whereClause);
      
      [assignments, totalAssignments] = await Promise.all([
        AssignmentModel.find(mongoWhere)
          .populate({
            path: 'sectionId',
            model: 'Section',
        ...
        AssignmentModel.countDocuments(mongoWhere)
      ]);
      ...
      [assignments, totalAssignments] = await Promise.all([
          ...
          include: [
            {
              model: SectionModel,
              as: 'section',
              attributes: ['name', 'code', 'batch', 'semester']
            }
          ],
        ...
        AssignmentModel.count({ where: whereClause })
      ]);

      // Get submission counts
      for (let assignment of assignments) {
        const submissionCount = await AssignmentSubmissionModel.count({
          where: { assignmentId: assignment.id }
        });
        assignment.dataValues.submissionCount = submissionCount;
      }
    }
    ...
    await LoggingService.log({
      level: 'info',
      message: `Faculty ${faculty.id} viewed assignments list`,
      userId: facultyUserId,
      details: {
        assignmentsCount: assignments.length,
        totalCount: totalAssignments,
        filters: { status, sectionId, search },
